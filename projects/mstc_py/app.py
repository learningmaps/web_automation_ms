...
def run_mstc():
    # Import locally to avoid circular dependencies or early loads
    from mstc_py.scraper import scrape_links
    
    supabase = get_supabase()

    # --- HEADER, STATS & CONTROLS ---
    # Fetch stats first to display them
    stats = supabase.schema("mstc").table("processed_pdfs").select("status", count="exact").execute()
    ...
    with tab1:
        ...
        urls_resp = supabase.schema("mstc").table("processed_pdfs").select("*").order("discovered_at", desc=True).execute()
        if urls_resp.data:
    ...
    with tab2:
        ...
        blocks_resp = supabase.schema("mstc").table("mine_block_summaries").select("*, processed_pdfs(discovered_at, file_id, pdf_url)").execute()
        if blocks_resp.data:
    ...
    with tab3:
        ...
        blocks_nit_resp = supabase.schema("mstc").table("tender_blocks").select(blocks_query).execute()
